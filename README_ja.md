...
| 🛡️ **エラー分類** | IP 制限 / ドメイン無効 / ログイン無効 / クォータ枯渇 / ネットワーク異常をすべて分類して処理し、わかりやすいメッセージを表示します。プラグインがクラッシュすることはありません |
...
**方法 2：手動インストール**
1. プラグインフォルダを `/AstrBot/data/plugins/` に置きます
2. AstrBot を再起動します
3. 管理パネルで必要なパラメータを設定します
...
> 💡 中国大陸のサーバーでは `proxy` にプロキシ（例：`http://127.0.0.1:7897`）を指定する必要があります。海外サーバーでは空欄で構いません。安定した onion アクセスを使いたい場合は [Tor/onion 導入](#-toronion-導入) を参照してください。
