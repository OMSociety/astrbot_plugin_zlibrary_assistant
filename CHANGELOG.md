...
维护性更新：隐患修复 + 代码卫生，无功能变化。

### 修复
- `requirements.txt` 补声明 `aiofiles`（此前仅声明 aiohttp，干净环境安装插件会因缺依赖报错）
- 域名配置规范化：自动剥掉 `http(s)://` 协议前缀与多余斜杠（防止从浏览器复制的带协议域名拼出 `https://https://…` 无效 URL）
- 下载文件名清洗：替换 Windows 非法字符（`\ / : * ? " < > |`）与控制字符，避免保存文件失败
- 移除 `@register` 装饰器：插件标识统一由 `metadata.yaml` 驱动（消除装饰器与 metadata 的 name/version 不一致）

### 优化
- `initialize()` 增加防重入守卫（重复初始化时跳过已在运行的登录任务）
- 删除 `_request_json` 死参数 `allow_html`
- ruff 规范化导入排序；`.gitignore` 排除 `.ruff_cache/`
