...
- **封面下载加固**：封面先经 Pillow 缩放压缩（卡片显示宽仅 84px，体积降 90%+，渲染更快）再内嵌；按文件头魔数判断真实图片类型（Z-Library CDN 常返回缺失/错误的 Content-Type，此前可能按错类型解码导致图片不显示）；下载成败写入日志（INFO 汇总 + DEBUG 明细）便于排查
- **封面下载兜底**：封面 CDN 域名（如 covers.zlibcdn2.com）经常不可达导致下载失败，现支持原 URL 失败后自动替换域名（z-lib.fm）重试；相对路径封面自动拼完整 URL；失败原因升级为 WARNING 日志（含 URL 与原因）
- **封面下载带登录态**：封面 CDN（covers.z-lib.sk）要求登录 cookies，裸请求返回 HTTP 513；现封面下载统一携带账号池**第一个账号**的 remix 凭证（无账号时匿名请求）
- **封面下载简化**：移除多域名替换重试（封面 CDN 对特定出口 IP/节点风控属服务器侧问题，替换域名无济于事），只下载原 URL；失败日志合并为一条 WARNING（含示例原因），不再每张封面刷屏
