...
- **Tor/onion 访问（可选）**：onion 地址仅允许 v3 格式且必须经 SOCKS 访问；重定向仍逐跳执行 SSRF 校验。(#1)
- 下载改为边读边限长，避免异常响应耗尽 AstrBot 内存。(#1)
...
- 可选通过 Tor SOCKS 访问 v3 onion E-API，原有明网域名和 HTTP 代理配置保持兼容。(#1)
- 提供可选 Tor Docker sidecar，支持使用现有 HTTP/SOCKS 服务作为 Tor 上游代理。(#1)
- 新增可配置的单文件下载体积上限。(#1)
